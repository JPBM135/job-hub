...
export function verifyAuthToken<T>(token: string): VerifyAuthTokenResult<T> {
  try {
    ...
    const [encodedHeader, encodedUser, hmac] = noBearer.split('.') as [
      string,
      string,
      string,
    ];

    if (!encodedHeader || !encodedUser || !hmac)
    ...
    if (!data.data || !data.exp || !data.iat || !data.iss)
      return { data: null, valid: false };
    if (data.iss !== 'jobhub') return { data: null, valid: false };
    if (dayjs(data.exp * 1_000).isBefore(dayjs()))
      return { data: null, valid: false };
    if (dayjs(data.iat * 1_000).isAfter(dayjs()))
      return { data: null, valid: false };
